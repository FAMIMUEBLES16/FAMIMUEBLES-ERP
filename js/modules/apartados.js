...
const balance = item => Math.max(0, item.total - item.initial - item.paid);
...
export function renderApartados(data) {
  ...
  return page('CARTERA','Apartados','<button class="primary" data-action="payment">＋ Registrar abono</button>',
    `${table(['Apartado','Cliente','Fecha','Total','Inicial','Abonado','Pendiente','Estado','Acciones'], apartados.map(item => `<tr><td><strong>${item.id}</strong></td><td>${item.customer}</td><td>${item.date}</td><td>${money(item.total)}</td><td>${money(item.initial)}</td><td>${money(item.paid)}</td><td>${money(balance(item))}</td><td>${badge(item.status)}</td><td><button class="table-action" data-action="apartado-payment" data-apartado-id="${item.id}">Registrar abono</button>${isAdmin?`<button class="table-action" data-action="edit-apartado" data-apartado-id="${item.id}">Editar</button><button class="table-action danger-text" data-action="delete-apartado" data-apartado-id="${item.id}">Eliminar</button>`:''}</td></tr>`))}`);
}