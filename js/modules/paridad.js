...
export function payrollModal(data = {}) {
  ...
  const employees = JSON.stringify([...sellers.values()]).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
  const employeeList = [...sellers.values()].map(employee => `<li>${employee.empleado} <span class="muted">· Salario base: ${employee.salario_base}</span></li>`).join('');
  return `<div class="modal-backdrop"><form class="modal" id="parity-payroll-form"><button type="button" class="modal-close">x</button><p class="eyebrow">NOMINA</p><h2>Calcular nómina</h2><label class="input-label">Fecha inicial<input class="field" name="start" type="date" required></label><label class="input-label">Fecha final<input class="field" name="end" type="date" required></label><label class="input-label">Local<select class="field" name="local" required>${stores}</select></label><label class="input-label">Comisión (%)<input class="field" name="rate" type="number" min="0" max="100" step="0.01" value="0"></label><fieldset class="input-label"><legend>Vendedores detectados</legend><ul class="detected-employees">${employeeList || '<li class="muted">No hay vendedores detectados.</li>'}</ul></fieldset><input type="hidden" name="employees" value="${employees}"><p class="muted">Se usan las ventas del período y la configuración de salario guardada para cada vendedor.</p><div class="quick-actions"><button class="outline" type="button" data-action="parity-payroll-preview">Previsualizar</button><button class="primary" type="submit">Cerrar nómina</button></div><div data-parity-result></div></form></div>`;
}
