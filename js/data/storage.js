...
function normalizeState(state) {
  ...
  const productsBySourceId = new Map(base.products.map(product => [String(product.sourceId || product.codigo || product.code || ''), product]));
  base.inventory = Array.isArray(base.inventory) ? base.inventory.map(item => { const product = productsById.get(String(item.productId)) || productsBySourceId.get(String(item.productId)); return { ...item, productId:product?.id || item.productId, storeId:item.storeId || storesByName[item.store] || base.stores[0]?.id }; }) : [];
  base.inventoryByStore = Array.isArray(base.inventoryByStore) ? base.inventoryByStore.map((item,index) => { const product=base.products.find(candidate=>candidate.id===item.productId); const minimumStock=Number(item.minimumStock ?? item.minimumQuantity ?? item.minimum ?? product?.minimum ?? 3); const maximumStock=Number(item.maximumStock ?? item.maximum ?? product?.maximum ?? minimumStock * 3); const quantity=Math.max(0, Number(item.quantity || 0)); return { ...item, id:item.id || `INV-${String(index + 1).padStart(5,'0')}`, quantity, minimumStock, minimum:minimumStock, maximumStock, maximum:maximumStock, reservedStock:Number(item.reservedStock || 0), updatedAt:item.updatedAt || new Date().toISOString() }; }) : [];
  base.inventoryByStore = Array.isArray(base.inventoryByStore) ? base.inventoryByStore.map((item,index) => { const product=productsById.get(String(item.productId)); const minimumStock=Number(item.minimumStock ?? item.minimumQuantity ?? item.minimum ?? product?.minimum ?? 3); const maximumStock=Number(item.maximumStock ?? item.maximum ?? product?.maximum ?? minimumStock * 3); const quantity=Math.max(0, Number(item.quantity || 0)); return { ...item, id:item.id || `INV-${String(index + 1).padStart(5,'0')}`, quantity, minimumStock, minimum:minimumStock, maximumStock, maximum:maximumStock, reservedStock:Number(item.reservedStock || 0), updatedAt:item.updatedAt || new Date().toISOString() }; }) : [];
  const inventoryKeys = new Set(base.inventoryByStore.map(item => `${item.productId}\u0000${item.storeId}`));
  base.inventory.forEach(item => { const key=`${item.productId}\u0000${item.storeId}`; if (!inventoryKeys.has(key)) { base.inventoryByStore.push({ id:nextId(base.inventoryByStore,'INV'), productId:item.productId, storeId:item.storeId, quantity:Number(item.quantity || 0), minimumStock:Number(item.minimum || item.ideal || 0), updatedAt:new Date().toISOString() }); inventoryKeys.add(key); } });
  const productsWithInventory = new Set(base.inventoryByStore.map(item => String(item.productId)));
  base.products.forEach(product => { if (!productsWithInventory.has(String(product.id))) base.inventoryByStore.push({ id:nextId(base.inventoryByStore,'INV'), productId:product.id, storeId:base.stores[0]?.id, quantity:Number(product.stock || 0), minimumStock:Number(product.minimum || 0), updatedAt:new Date().toISOString() }); });
