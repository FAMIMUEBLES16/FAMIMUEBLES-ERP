...
function openSaleEditModal(saleId){const sale=(store.collection.sales||[]).find(item=>String(item.id)===String(saleId)) || (store.collection.ventas||[]).find(item=>String(item.id)===String(saleId));if(!sale)return;const customers=store.collection.customers.map(item=>`<option value="${item.id}" ${String(item.id)===String(sale.customerId)?'selected':''}>${item.name}</option>`).join('');const saleDate=String(sale.date||sale.fecha||sale.fecha_venta||sale.created_at||sale.createdAt||'').slice(0,10)||localDateValue();$('#modal-root').innerHTML=`<div class="modal-backdrop"><form class="modal" id="sale-edit-modal"><button type="button" class="modal-close">×</button><p class="eyebrow">ADMINISTRACION</p><h2>Editar venta ${sale.id}</h2><label class="input-label">Fecha de venta<input class="field" type="date" name="date" value="${saleDate}" required></label><label class="input-label">Cliente<select class="field" name="customerId">${customers}</select></label><label class="input-label">Metodo de pago<select class="field" name="paymentMethod"><option ${sale.paymentMethod==='Efectivo'?'selected':''}>Efectivo</option><option ${sale.paymentMethod==='Transferencia'?'selected':''}>Transferencia</option><option ${sale.paymentMethod==='Tarjeta'?'selected':''}>Tarjeta</option><option ${String(sale.paymentMethod).toLowerCase().includes('credito')?'selected':''}>Crédito interno FAMIMUEBLES</option><option ${sale.paymentMethod==='Apartado'?'selected':''}>Apartado</option></select></label><button class="primary wide">Guardar cambios</button></form></div>`;$('.modal-close').onclick=()=>$('#modal-root').innerHTML='';$('#sale-edit-modal').onsubmit=async event=>{event.preventDefault();try{const values=Object.fromEntries(new FormData(event.target));await persistCatalogRecord('/api/catalog/sale', {...sale,...values},'PUT');const normalized=await hydrateState();if(normalized){store.state=normalized;await hydrateCatalog(store.state);}$('#modal-root').innerHTML='';render();showToast('Venta actualizada correctamente.');}catch(error){showToast(error.message,'error');}};}
